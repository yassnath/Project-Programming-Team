...
function operasi(op) {
  if (operator!== null) kalkulasi();
  operator = op;
  angkaPertama = angkaKedua;
  angkaKedua = '';
  display.innerHTML = `${angkaPertama} ${op} ${angkaKedua}`;
}

function kalkulasi() {
  let hasil;
  const pertama = parseFloat(angkaPertama);
  const kedua = parseFloat(angkaKedua);
  ...
  display.innerHTML = `${angkaPertama} ${operator} ${angkaKedua} = ${hasil}`;
  angkaKedua = hasil;
  operator = null;
}
...
function hapusAngka() {
  if (angkaKedua.length > 0) {
    angkaKedua = angkaKedua.slice(0, -1);
    display.innerHTML = angkaKedua;
  }
}

let del = document.getElementById('del');
del.addEventListener('click', hapusAngka);
